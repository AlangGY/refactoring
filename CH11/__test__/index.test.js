...
describe('alert for miscreant', () => {
    test('', () => {
        const returnValue = module.alertForMiscreant(['조커', '사루만', '배트맨']);
        expect(returnValue).toBe('조커');
        expect(alertModule.setOffAlarms).toHaveBeenCalled()
    });

    test('', () => {
        const returnValue = module.alertForMiscreant([ '사루만','조커', '배트맨']);
        expect(returnValue).toBe('사루만');
        expect(alertModule.setOffAlarms).toHaveBeenCalled()
    });

    test('', () => {
        const returnValue = module.alertForMiscreant([ '할리퀸','슈퍼맨', '배트맨']);
